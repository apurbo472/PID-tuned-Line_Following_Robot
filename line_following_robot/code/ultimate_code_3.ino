...
if (!whiteLineMode) {
  ...
  if ((   sensorValues[0] > OUTER_BLACK_THRESHOLD && sensorValues[1] > OUTER_BLACK_THRESHOLD)
    ...
    setMotor(10,10); delay(40);}


  // Right side strong, left side clear -> pivot RIGHT
  if ((sensorValues[6] > OUTER_BLACK_THRESHOLD && sensorValues[7] > OUTER_BLACK_THRESHOLD)
      && (sensorValues[0] < OUTER_WHITE_THRESHOLD)) {

    setMotor(60,-0); delay(60);

    //setMotor(MAX_SPEED,-MAX_SPEED); delay(5);
    ...
    while (millis() - pivotStart < SPECIAL_PIVOT_TIME) {   
      ...
      setMotor(BASE_SPEED, -BASE_SPEED );            // pivot RIGHT




      //setMotor(MAX_SPEED, -MAX_SPEED);            // pivot RIGHT



      qtra.readCalibrated(sensorValues);
      for (int i = 2; i <= 5; i++)
        if (sensorValues[i] > OUTER_BLACK_THRESHOLD) { setMotor(0,0); lastError = 0; return; }
    }
    lastError = 0;
    return;
  }

  // LEFT side strong, right side clear -> pivot LEFT
  if ((sensorValues[0] > OUTER_BLACK_THRESHOLD && sensorValues[1] > OUTER_BLACK_THRESHOLD)
      && ( sensorValues[7] < OUTER_WHITE_THRESHOLD)) {

    setMotor(-0, 60); delay(60);
    ...
    while (millis() - pivotStart < SPECIAL_PIVOT_TIME) {
      ...
      //setMotor(-MAX_SPEED, MAX_SPEED);           // pivot LEFT
